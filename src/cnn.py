...
class cnn:
    ...
    def train_model(self, X_train, y_train, X_val, y_val, epochs=20, batch_size=128):
        ...
        if self.multi_target:
            search.test_model(self.model, X_train, y_train, X_val, y_val, num_combs=3)
        else:
            search.test_model(self.model, X_train, y_train, X_val, y_val, get_weight_dict(y_train), num_combs=3)
